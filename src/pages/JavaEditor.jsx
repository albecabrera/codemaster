...
function JavaEditor() {
  ...
  const challenges = getChallengesByLanguage('java');
  ...
  const handleRun = () => {
    ...
    setOutput(`${t('editor.output.compiling')}\n`);
    setTimeout(() => {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      <div className="p-4 border-b border-gray-200 dark:border-gray-700 space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
            <span className="px-2 py-0.5 bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400 rounded text-xs font-medium">{t(`editor.difficulties.${currentChallenge.difficulty}`)}</span>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        <AnimatePresence>
          {showHints && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mt-2 p-3 bg-red-50 dark:bg-red-900/20 rounded-lg overflow-hidden">
      ...
      <div className="p-4 border-t border-gray-200 dark:border-gray-700 flex gap-2">
         <Button onClick={handleRun} className="flex-1 bg-red-600 hover:bg-red-700 text-white shadow-md">
  ...
  return (
    <>
      <Helmet><title>{t('nav.java')} - {t('common.appName')}</title></Helmet>
      <EditorContainer
        ...
        <div className="flex flex-col h-full relative">
          ...
          <div className="flex-1 flex relative">
            {editorState.showLineNumbers && (
              ...
              >
                {code.split('\n').map((_, i) => (
                  <div key={i} className={editorState.highlightedLine === i ? 'text-red-500 font-bold' : ''}>{i + 1}</div>
                ))}
              </div>
            )}
            <textarea
            ...
            {editorState.highlightedLine !== null && (
               <div 
                 className="absolute left-0 right-0 pointer-events-none bg-red-500/20 dark:bg-red-400/10 transition-opacity duration-300"
